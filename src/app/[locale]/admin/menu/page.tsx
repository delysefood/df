...
import { Plus, Edit, Trash2, X, Save, Loader2, Image as ImageIcon, Upload, Camera, ArrowLeft, Heart, Sparkles, ChefHat, Star } from 'lucide-react';
...
export default function AdminMenuPage() {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    isAvailable: true,
    extras: [],
    sauceSettings: { hasSauces: false, maxSauces: 1, maxExtras: 12 }
  });
  ...
  const handleEdit = (item: any) => {
    ...
    setFormData({
      ...
      extras: item.extras || [],
      sauceSettings: item.sauceSettings || { hasSauces: false, maxSauces: 1, maxExtras: 12 }
    });
    setIsModalOpen(true);
  };
  ...
  const addExtra = () => {
    setFormData({
      ...formData,
      extras: [...formData.extras, { name: '', price: 0, isPopular: false }]
    });
  };
            ...
            >
              <div className="p-8 md:p-12 border-b border-border flex justify-between items-center bg-foreground/[0.02]">
                <div>
                  ...
                  <p className="text-foreground/40 text-[10px] uppercase font-black tracking-widest mt-1">Personnalisation Avancée</p>
                </div>
                      ...
                      {LANGUAGES.map((lang) => (
                        <div key={lang} className="space-y-4 p-6 rounded-3xl bg-foreground/5 border border-border group-within:border-gold/30 transition-colors">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-foreground/40">{lang.toUpperCase()}</span>
                          </div>
                          <input 
                            type="text"
                            placeholder={`Nom`}
                  ...
                  <div className="space-y-12">
                    ...
                    <div className="p-8 rounded-[2rem] bg-blue-500/[0.03] border border-blue-500/20 space-y-6">
                       <div className="flex items-center justify-between">
                          <h4 className="text-sm font-black text-foreground tracking-tight flex items-center gap-3"><Heart size={18} className="text-blue-500" /> Sauces</h4>
                          <button 
                            type="button"
                            onClick={() => setFormData({...formData, sauceSettings: {...formData.sauceSettings, hasSauces: !formData.sauceSettings.hasSauces}})}
                            className={`w-12 h-7 rounded-full relative transition-all ${formData.sauceSettings.hasSauces ? 'bg-blue-500' : 'bg-foreground/10'}`}
                          >
                            <div className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-all ${formData.sauceSettings.hasSauces ? 'left-6' : 'left-1'}`} />
                          </button>
                       </div>
                       {formData.sauceSettings.hasSauces && (
                         <div className="flex items-center gap-4 pt-4 border-t border-blue-500/10">
                            <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40">Limite:</label>
                            <input 
                              ...
                              className="w-16 bg-background border border-blue-500/20 rounded-xl px-3 py-2 text-center font-black text-blue-500"
                            />
                         </div>
                       )}
                    </div>
                    ...
                    {/* Extras */}
                    <div className="p-8 rounded-[2rem] bg-gold/[0.03] border border-gold/20 space-y-8">
                       <div className="flex items-center justify-between">
                          <h4 className="text-sm font-black text-foreground tracking-tight flex items-center gap-3"><Plus size={18} className="text-gold" /> Suppléments</h4>
                          <div className="flex items-center gap-4">
                             <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40">Max:</label>
                             <input 
                               type="number" min="1" max="50"
                               value={formData.sauceSettings.maxExtras}
                               onChange={(e) => setFormData({...formData, sauceSettings: {...formData.sauceSettings, maxExtras: parseInt(e.target.value) || 12}})}
                               className="w-16 bg-background border border-gold/20 rounded-xl px-3 py-2 text-center font-black text-gold"
                             />
                             <button type="button" onClick={addExtra} className="bg-gold text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">Ajouter</button>
                          </div>
                       </div>

                       <div className="space-y-4">
                          {formData.extras.map((extra: any, index: number) => (
                            <div key={index} className="flex gap-3 items-center bg-background p-4 rounded-2xl border border-border">
                               <input 
                                 type="text" placeholder="Nom"
                                 value={extra.name}
                                 onChange={(e) => updateExtra(index, 'name', e.target.value)}
                                 className="flex-1 bg-foreground/5 border-none rounded-xl px-4 py-3 text-xs font-bold"
                               />
                               <input 
                                 type="number" step="0.5" placeholder="€"
                                 value={extra.price}
                                 onChange={(e) => updateExtra(index, 'price', e.target.value)}
                                 className="w-20 bg-foreground/5 border-none rounded-xl px-4 py-3 text-xs font-black text-gold"
                               />
                               <button 
                                 type="button"
                                 onClick={() => updateExtra(index, 'isPopular', !extra.isPopular)}
                                 className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${extra.isPopular ? 'bg-gold text-white' : 'bg-foreground/5 text-foreground/20'}`}
                                 title="Indiquer comme populaire"
                               >
                                  <Star size={16} className={extra.isPopular ? 'fill-current' : ''} />
                               </button>
                               <button type="button" onClick={() => removeExtra(index)} className="text-red-500 p-2"><Trash2 size={16}/></button>
                            </div>
                          ))}
                       </div>
                    </div>
              ...
              <div className="p-8 md:p-12 border-t border-border flex justify-end gap-6 bg-foreground/[0.02]">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-10 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest text-foreground/40">Annuler</button>
                <button form="menu-form" type="submit" disabled={saving} className="px-12 py-5 bg-gold text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3">
